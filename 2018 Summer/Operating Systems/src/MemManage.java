...
import abcmod.evschedsimul.ESOutputSet;
import abcmod.evschedsimul.EvSched;
...
class MemManage extends EvSched  // Event Scheduling Simulation program
{
    ...
    private final int EndPageFault = 1; // End of page fault
    private final int EndMemoryAccess = 2; // End of memory access
    /* Data Modules for implementing timing maps */
    private Poisson memoryAccesses;  // for defining number of memory accesses by a process
    /* Resource Entity - the kernel */
    private Kernel kernel;
    /* Aggregate Entities */
    private ConcurrentLinkedQueue<Process> readyQueue;    // process ready queue
    private HashSet<Process> ioQueue;    // IO queue implemented as a set (to make code clearer)
    /* Output Sets */
    ...
    public MemManage(PagingAlgorithm pgAlg, double t0time, double tftime, Seeds sd) {
        ...
        // mean number of memory accesses per process execution cycle
        int meanMemAccesses = 20;
    ...
    private void endMemoryAccess()    // endMemoryAcess SCS
    {
        ...
        if (kernel.processExecuting.numMemAccess <= 0) {
            ...
            kernel.processExecuting = readyQueue.poll();
    ...
    private void preConditions() {
        while (true)  // loop until no preconditions are true
        {
            if (kernel.action == Kaction.PAGEFAULT) {
                ...
                kernel.processExecuting = readyQueue.poll(); // Next process
                if (kernel.processExecuting == null)
                ...
                // Time for faults 100 time units
                double faultTime = 100;
    ...
    private int selectAddress(Process prc)  // returns virtual page number being addressed
    {
        return prc.selectAddressPrc();
    } // defined as part of the class

    private Kaction nextAction(int vpage, Process prc) {
...
class Process {
    int numPages;   // Number of virtual pages
    PgTblEntry[] pageTable;   // Page table
    int[] workingSet;   // List of virtual pages in working set
    private int[] wsSegments;  // defines indexes in workingSet to divide into segments (code, data, etc.)
    int vpage;     // virtual page being accessed
    int vpageFromFault;     // virtual page just loaded after default - need to be accessed first executed.
    //-----------------------------------------
    ...
    private Poisson prcMA2ChangeWS;   // for getting a discrete random number
    private Uniform prcDiscreteRandom;   // for getting a discrete random number
    private Binomial prcBern;     // To create Bernouilli random number generator
    ...
    Process(int pid, int nCode, int nData, int nStack, int nHeap, int meanMA, int sd1, int sd2, int sd3) {
        int i;  // an index
    ...
    protected boolean areAllocatedFramesFull() {
        if (allocatedFrames == null) return (false);
        return allocatedFrames.length >= numAllocatedFrames;
...
class Kernel {
    ...
    Process processExecuting;    // executing process
    Kaction action;                // Action to be taken - set to:
    ...
    Kernel(int numFrames, PagingAlgorithm pgAlg) {
        int i;

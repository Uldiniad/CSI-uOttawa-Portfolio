public class KernelFunctions {
    ...
    static void pageReplacement(int vpage, Process prc, Kernel krn) {
    ...
    // DO NOT CHANGE this method
    private static void addPageFrame(int vpage, Process prc, Kernel krn) {
    ...
    private static void pageReplAlgorithm(int vpage, Process prc, Kernel krn) {
    ...
    static void doneMemAccess(int vpage, Process prc, double clock) {
    ...
    private static void pageReplAlgorithmFIFO(int vpage, Process prc) {
    ...
    private static int findvPage(PgTblEntry[] ptbl, int fr) {
        int i;
    ...
    private static void pageReplAlgorithmCLOCK(int vpage, Process prc) {
        int vPageReplaced; // Page to be replaced
        int frame; // frame to receive new page
        boolean flag = true;
        while (flag) {
    ...
    private static void pageReplAlgorithmLRU(int vpage, Process prc) {
        int vPageReplaced = 0;  // Page to be replaced
        // Find page to be replaced
        ...
        double timeStamp = prc.pageTable[vPageReplaced].tmStamp;
        for (int i = 0; i < prc.pageTable.length; i++) {
            if (prc.pageTable[i].valid && timeStamp > prc.pageTable[i].tmStamp) {
                timeStamp = prc.pageTable[i].tmStamp;
                vPageReplaced = i;
            }
        }
        prc.pageTable[vPageReplaced].valid = false;  // Old page is replaced.
    ...
    private static void pageReplAlgorithmCOUNT(int vpage, Process prc) {
        int vPageReplaced = 0;  // Page to be replaced
        // Find page to be replaced
        for (int i = 0; i < prc.pageTable.length; i++) {
            if (prc.pageTable[i].valid) {
        ...
        double count = prc.pageTable[vPageReplaced].count;
        for (int i = 0; i < prc.pageTable.length; i++) {
            if (prc.pageTable[i].valid && count > prc.pageTable[i].count) {

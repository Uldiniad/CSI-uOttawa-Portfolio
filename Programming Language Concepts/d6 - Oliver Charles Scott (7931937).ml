...
let sublist_starting_at_index (n:int) (l: 'a list) : 'a list =
let rec find_sublist_starting_at_index (n:int) (l: 'a list) : 'a list =
    match l with
    | [] -> []
    | h::t -> if n == 1 then h::t else find_sublist_starting_at_index (n-1) t
in if (n < 1 || n > List.length l) then raise NoSuchSublist else find_sublist_starting_at_index n l
;;
...
(* Problem 1(b). Now consider the definition of the recursive function
   "find" below and consider a call to the function with 1 as the
...
try (if n < 0 then raise IndexOutOfBounds else find p 1 (sublist_starting_at_index n l))
with
| NotFound -> 0
| NoSuchSublist -> -1
| IndexOutOfBounds -> -2
...
find2 p n l (fun n -> n*2) (fun() -> print_endline "NotFound"; 0);;
...
let delay_list = [ref (UN (fun () -> 1)); ref (UN (fun () -> 2)); ref (UN (fun () -> 3)); ref (UN (fun () -> 4)); ref (UN (fun () -> 5)); ref (UN (fun () -> 6))];;
...
match e with
| Number x -> new number_exp x
| Sum(x,y) -> new sum_exp (exp2expression x) (exp2expression y)

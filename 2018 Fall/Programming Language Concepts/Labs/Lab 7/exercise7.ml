...
(* On peut représenter des expressions générées par la grammaire:
   ...
   incluent (1) un prédicat "is_atomic" que indique si l'argument a
   des sous-expressions ou non, (2) et (3) des opérations pour
   récupérer les sous-expressions de guache et de droite (si
...
(* Dans une expression conditionnelle a?b:c, on évalue "a" et si la
   ...
   Encore une fois, essayez de faire le moins de modifications possible.
   Si nécessaire, redéfinissez la hiérarchie de classes créée pour la
   question 2 de sorte qu'elle soit plus générale et puisse traiter des
   opérations unaires, binaires et ternaires.
 *)
...
(* QUESTION 4. Redéfinissez à nouveau la hiérarchie complète. Cette
   ...
   (x: int) et modifie un objet de la class "expression" de sorte que
   toutes ses feuilles soient incrémenté par la valeur de x.  (Les
   ...
   pas créer une nouvelle instance d'une "expression". Il devrait
   modifier les instances auxquelles il est appliqué.
